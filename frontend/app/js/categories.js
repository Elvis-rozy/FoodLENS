...
window.addEventListener("DOMContentLoaded", () => {
  //fetch API data containing all food details
  ...
  .then(data => {
    ...
    //function that loads the category links
    function displayMenuButton() {
      ...
      buttons.forEach((button) => {
        button.addEventListener("click", (e) => {
          ...
          //condition to determine which items are displayed based on selected category
          if (filter == "All") {
            displayMenuItem(data);
          } else  {
...
//function that displays every other category
function displayMenuCategory(menuItems) {
  let displayMenu = menuItems.map((item) => {
    return `<div id="${item.category}" class="menu-item">
        <img src=${item.img_url} class="photo" alt="">
        <div class="item-info">
          <h4 id="${item.food_name}">${item.food_name}</h4>
          <p class="item-text">${item.cuisine}
  ...
  let arr = ["pastries", "starches", "meat-seafoods", "fruits", "vegetables"];
